...
def get_num_vib_modes(geometry, nofatoms):
    n_of_freqs = 0

    if 'linear' in geometry:
        n_of_freqs = (3.0*nofatoms)- 5
    if 'non-linear' in geometry:
        n_of_freqs = (3.0*nofatoms)- 6
    if 'choose' in geometry:
        n_of_freqs = nofatoms
    return int(n_of_freqs)
...
energies_in_eV(Outcar_frequency_data_list)
...
def zero_point_energy_w(tuple_frecuencies_in_wavenumbers):
...
def vibrational_entropy(temperature, vibrational_energies_in_eV):
    ...
    Beta = 1/(boltzmann_constant_eV_per_K[0]*temperature)
...
def _vibrational_entropy_contribution(energies, temperature): # function taken as it is from ase.thermochemistry
...
def get_ZPE_correction_eV(energies):
...
###defining Variables
ZPE = zero_point_energy_w( frecuencies_in_wavenumber( Outcar_frequency_data_list ) )
...
#print "RESULTS FROM GEOMETRY FUNCTION", get_num_vib_modes(geometry,nofatoms)
print " Temperature used:", Temperature
print "zero point energy (ZPE):\n", round( zero_point_energy_w( frecuencies_in_wavenumber( Outcar_frequency_data_list ) ),3 ),'eV'

print "Svib my function  eV/K    \n", vibrational_entropy(temperature=Temperature,
                                                   vibrational_energies_in_eV=frequencies_to_energies_in_eV(frecuencies_in_wavenumber(Outcar_frequency_data_list)))
print "T*Svib MF eV    \n ", TS, '\n'


''''NOTE: the list of energies use for ASE inputs as energies are taken as given from the outcar form the lists of eneegries that are liste in meV. 
Energies for my functions are converted from wavenumbers to eV.
numerically there is discrepancy between this values'''

print "ZPE from ASE:\n", get_ZPE_correction_eV( energies_in_eV( Outcar_frequency_data_list ) ),'\n'

print'Svib form ASE.thermochemstry function\n', _vibrational_entropy_contribution(energies_in_eV(Outcar_frequency_data_list),Temperature), '\n'
print "T*Svib ASE       \n", Temperature* _vibrational_entropy_contribution(energies_in_eV(Outcar_frequency_data_list),Temperature)
